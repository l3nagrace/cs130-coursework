...
const addShape = (ev) =>{
    ...
    if (shape === 'circle'){
        const circle = `<circle cx="${ev.offsetX}" cy="${ev.offsetY}" r="${size/2}" stroke="black" stroke-width="3" fill="${color}"></circle>`;
        document.querySelector('svg').insertAdjacentHTML("beforeend", circle);
        ...
        <rect 
            x="${ev.offsetX}" 
            y="${ev.offsetY}" width="${size}" height="${size}" stroke="black" stroke-width="3" fill="${color}"></rect>'
        document.querySelector('svg').insertAdjacentHTML("beforeend", square);
    } 
